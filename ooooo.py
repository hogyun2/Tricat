...
class pathplanner():
    ...
    def Obstacle_detector(self,data):
        ...
        self.obstacles=[[i.center.y,-i.center.x,i.radius] for i in data.circles]
        self.obstacles.append([2,0,0.1])
        self.obstacles.append([-2,0,0.1])
    ...
    def dist_to_obstacles(self):
        ...
        for i in self.obstacles:
            dist_ob=math.sqrt(math.pow((i[0]-self.cur_x)+(i[1]-self.cur_y),2)
            angle_ob_st=math.degrees(math.atan2(i[0]-self.cur_x,i[1]-self.cur_y)-math.atan2(i[2],dist_ob))
            angle_ob_ed=math.degrees(math.atan2(i[0]-self.cur_x,i[1]-self.cur_y)+math.atan2(i[2],dist_ob))
            if abs(angle_ob_st)<100 and abs(angle_ob_ed)<100:
                pass
            else:
                obob.append([angle_ob_st,angle_ob_ed,dist_ob])
        obob.sort()

        return obob

    def calc_safezone(self,obob):
        safezone=[[-0.1,0.1,0.1,0.1]]
        for i in range(1,len(obob)):
            safezone.append([obob[i-1][1],obob[i-2][0],obob[i-2][2],obob[i-1][2]])
            #                theta ed     theta st    d st         d ed
        area=0
        max_safearea=safezone[0]
        for i in safezone:
            area=abs(0.5*i[2]*i[3]*math.sin(math.radians(i[0]-i[1])))
            if max_safearea<area:
                max_safearea=i
    ...
    def global_path(self,error_N):
        print('global_path')
        self.thruster_pub.publish(1800-self.dist_to_goal*20)
        self.servo_pub.publish(round((90+error_N*0.1),0))
        time.sleep(0.5)
        

    #self.obstacles=[[x,y,r],[x,y,r]...]
    #obob=[[dist,angle]]        
    def local_path(self,error_N,max_safearea):
        print('local_path')
        heading=(max_safearea[1]*max_safearea[3]+max_safearea[0]*max_safearea[2])/(max_safearea[2]+max_safearea[3])
        self.thrusterL_pub.publish(1600)
        self.thrusterR_pub.publish(1600)
        self.servo_pub.publish(90+heading*0.1)
        #print(heading)
        time.sleep(0.5)    
