...
class pathplanner():
    ...
    def Obstacle_detector(self,data):
        for i in data.circles:
            self.obstacles.append([i.center.x,i.center.y,i.radius])
        #print(data.circles[0].center)
    ...
    def dist_to_obstacles(self):
        for i in self.obstacles:
            ...
            angle_ob=math.atan2((i[0]-self.cur_x),(i[1]-self.cur_y))*180/math.pi
            self.obob.append([dist_ob,angle_ob])
    ...
    def local_path(self,error_N):
        print('local_path')
        for i in self.obob:
            if i[0]<5 and abs(math.sin(i[1]*180/math.pi))<0.5:
                if self.shookshook[0]>i[0] and abs(self.shookshook[1])<120:
                    self.shookshook=i
        
        if 3<self.shookshook[0]<5:
            self.thruster_pub.publish(1650)
            if abs(self.shookshook[1])<45:
                if self.shookshook[1]<0:
                    self.servo_pub.publish(70)
                else:
                    self.servo_pub.publish(110)
            elif 45<abs(self.shookshook[1])<90:
                if self.shookshook[1]<0:
                    self.servo_pub.publish(80)
                else:
                    self.servo_pub.publish(110)
        ...
        elif self.shookshook[0]<1:
            self.servo_pub.publish(90)
            self.thruster_pub.publish(1400)
